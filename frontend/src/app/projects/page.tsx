...
export default function ProjectsPage() {
  ...
  const [showEditMemberSuggestions, setShowEditMemberSuggestions] = useState(false);
  ...
  const visibleProjects =
    currentUserRole === "ADMIN"
      ? projects
      : projects.filter(
          (project) => isProjectCreator(project) || isProjectAssignedToMe(project)
        );
  ...
  const getProjectStats = (projectId: number) => {
    ...
    const testing = projectTasks.filter((task) => task.status === "TEST").length;
  ...
  const projectStatusChartData = [
    ...
    {
      name: "In Progress",
      value: visibleProjects.filter((project) => project.status === "In Progress").length,
    },
    {
      name: "Pending",
      value: visibleProjects.filter((project) => project.status === "Pending").length,
    },
    {
      name: "Completed",
  ...
  const projectTaskChartData = visibleProjects.map((project) => ({
    name: project.name,
    tasks: getProjectTasks(project.id).length,
  }));

  const projectProgressChartData = visibleProjects.map((project) => ({
    name: project.name,
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="mb-8 rounded-3xl bg-gradient-to-r from-blue-600 to-cyan-500 p-8 text-white shadow-lg">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl bg-white/15 p-4 backdrop-blur">
            <p className="text-xs font-bold uppercase text-blue-50">Total Projects</p>
            <p className="mt-1 text-3xl font-black">{visibleProjects.length}</p>
          </div>

          <div className="rounded-2xl bg-white/15 p-4 backdrop-blur">
            <p className="text-xs font-bold uppercase text-blue-50">Project Tasks</p>
            <p className="mt-1 text-3xl font-black">{tasks.length}</p>
          </div>

          <div className="rounded-2xl bg-white/15 p-4 backdrop-blur">
            <p className="text-xs font-bold uppercase text-blue-50">Team Members</p>
      ...
      <div className="mb-8 grid gap-6 lg:grid-cols-2">
                ...
                >
                  {projectStatusChartData.map((entry, index) => (
                    <Cell
                      ...
                      fill={["#10b981", "#3b82f6", "#f59e0b", "#64748b"][index % 4]}
                    />
                  ))}
                </Pie>
                <Tooltip />
        ...
        <div className="rounded-3xl border border-cyan-100 bg-white p-6 shadow-md">
          <div className="mb-4">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Jira-like workload chart showing how many tasks each project has.
            </p>
          </div>
        ...
        <div className="rounded-3xl border border-indigo-100 bg-white p-6 shadow-md lg:col-span-2">
          <div className="mb-4">
            <h2 className="text-xl font-black text-slate-800">
              Project Progress Comparison
            </h2>
            <p className="mt-1 text-sm text-slate-500">
              Completion percentage calculated from DONE tasks inside each project.
            </p>
          </div>
      ...
      <div className="mb-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-md">
        <h2 className="mb-4 text-xl font-bold text-slate-800">
          Create New Project
        </h2>

        <p className="mb-4 text-sm text-slate-500">
          Create a project for selected teammates or assign it to an entire department.
        </p>
        ...
        <textarea
          className="mt-4 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100"
        ...
        <div className="mt-4">
          ...
          {selectedDepartment && (
            <p className="mt-2 rounded-xl bg-cyan-50 px-4 py-3 text-xs font-semibold text-cyan-700">
              All users in {selectedDepartment} department will be added to this project.
            </p>
          )}
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="mt-5 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 px-6 py-3 font-semibold text-white shadow-lg shadow-blue-500/25 transition hover:scale-[1.01] disabled:opacity-60"
        >
      ...
      {editingProject && canEditProject(editingProject) && (
        <div className="mb-8 rounded-3xl border border-amber-200 bg-amber-50/70 p-6 shadow-md">
          <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Update project details without removing its members or linked tasks.
              </p>
            </div>
            ...
            <button
              ...
              className="w-fit rounded-xl bg-white px-4 py-2 text-sm font-bold text-slate-600 shadow-sm hover:bg-slate-100"
            >
              Cancel Edit
            </button>
          </div>
          ...
          <div className="relative mt-4">
            ...
            {showEditMemberSuggestions && filteredEditUsers.length > 0 && (
              <div className="absolute z-20 mt-2 max-h-64 w-full overflow-y-auto rounded-2xl border border-amber-200 bg-white shadow-xl">
          ...
          <div className="mt-4">
            ...
            >
              <option value="">No department selected</option>
              {departments.map((department) => (
                <option key={`edit-department-${department}`} value={department}>
                  {department}
                </option>
              ))}
            </select>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            <button
              ...
              disabled={loading}
              className="rounded-xl bg-amber-500 px-6 py-3 font-semibold text-white shadow-lg shadow-amber-500/25 transition hover:bg-amber-600 disabled:opacity-60"
            >
            ...
            <button
              onClick={cancelEditProject}
              className="rounded-xl bg-white px-6 py-3 font-semibold text-slate-600 shadow-sm transition hover:bg-slate-100"
            >
              Cancel
            </button>
          </div>
      ...
      {currentUserRole !== "ADMIN" && (
        <div className="mb-6 grid gap-4 md:grid-cols-2">
          ...
          <div className="rounded-3xl border border-cyan-100 bg-cyan-50/70 p-5 shadow-sm">
            <p className="text-sm font-bold text-cyan-700">Assigned Projects</p>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {[...visibleProjects].sort((a, b) => b.id - a.id).map((project) => {
          ...
          <div
            key={project.id}
            className="group relative overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
          >
            ...
            <div className="relative z-10">
                ...
                <div className="flex flex-col items-end gap-2">
                  {currentUserRole !== "ADMIN" && (
                    <span className="whitespace-nowrap rounded-full bg-white px-3 py-1 text-xs font-semibold text-slate-600 border border-slate-100">
                      {isProjectCreator(project) ? "My Project" : "Assigned"}
                    </span>
                  )}
              ...
              <div className="mt-5 rounded-2xl border border-slate-100 bg-slate-50 p-4">
                ...
                <div className="mt-4 grid grid-cols-3 gap-2 text-center text-xs">
                  <div className="rounded-xl bg-white p-2">
                    <p className="font-black text-slate-800">{stats.total}</p>
                  ...
                  <div className="rounded-xl bg-white p-2">
                    <p className="font-black text-emerald-700">{stats.done}</p>
              ...
              <div className="mt-5">
                ...
                {project.memberEmails && project.memberEmails.length > 0 ? (
                  ...
                  <p className="text-sm text-slate-400">No members added.</p>
                )}
              </div>
              ...
              <div className="mt-8 flex items-center justify-between text-sm">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="rounded-xl bg-slate-100 px-3 py-1 font-medium text-slate-600 transition hover:bg-slate-200"
                  >
                    Details
                  </button>
